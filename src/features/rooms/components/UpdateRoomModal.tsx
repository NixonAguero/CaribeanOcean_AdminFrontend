...
interface Props {
  ...
  onClose: () => void;
  room: Room | null;
  onUpdate: (id: number, number: number, active: boolean) => Promise<void>;
}

export default function UpdateRoomModal({ isOpen, onClose, room, onUpdate }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await onUpdate(room.id, parseInt(number), active);
      onClose();
    } catch {
      ...
      >
        <div className={styles.modalHeader} style={{ padding: "0 0 16px 0", borderBottom: "none", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
            ...
            <h2 className={styles.sectionLabel} style={{ fontSize: "20px" }}>Update Room #{room.number}</h2>
          </div>
